 ...
 */
import {FAKE_PUBLICATIONS, FAKE_TEAM_INFO, FAKE_TEAM_MEMBERS} from "./fakeData";
...
if (env.REACT_APP_DEPLOYING) {
     TEAM_PUBLICATIONS = env.REACT_APP_TEAM_PUBLICATIONS ? JSON.parse(env.REACT_APP_TEAM_PUBLICATIONS) : [];

    /// The team information object consists of team email, orgName, teamName and twitterHandle
    TEAM_INFO = env.REACT_APP_TEAM_INFO ? JSON.parse(env.REACT_APP_TEAM_INFO) : null;

    /// The list of team members the team has created for rendering in the team members page
    TEAM_MEMBERS = env.REACT_APP_TEAM_MEMBERS ? JSON.parse(env.REACT_APP_TEAM_MEMBERS) : [];
}
else {
